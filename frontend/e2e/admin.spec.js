...
test('real login, catalog, team editor, enrollment, match, reload and logout', async ({ page, context }) => {
  const errors = [];
  page.on('pageerror', error => errors.push(error.message));
  const cookie = (await context.cookies()).find(item => item.name === 'vertice_token');
  expect(cookie?.httpOnly).toBe(true);
  await expect(page.locator('form')).toHaveCount(0);
  const teamsResponse = await page.request.get('/api/equipos/');
  expect(teamsResponse.status()).toBe(200);
  const initialTeams = await teamsResponse.json();
  await expect(page.locator('.v-metric').filter({ hasText: 'Clubes y selecciones' }).locator('strong'))
  ...
  const conf = await create('confederaciones', { nombre: `CI conf ${suffix}`, logo: 'https://example.test/logo.svg' });
  const comp = await create('competiciones', {
    nombre: `CI liga ${suffix}`, logo: 'https://example.test/logo.svg',
    tipo: 'liga_nacional', pais: 'Colombia', confederacion_id: conf.id,
  });
  const visitor = await create('equipos', {
    nombre: `CI visita ${suffix}`, logo: 'https://example.test/logo.svg',
    tipo: 'club', pais: 'Colombia', confederacion_id: conf.id,
  });
  const enrolled = await page.request.post(`/api/equipos/${visitor.id}/matricular/${comp.id}`);
  ...
  await dialog.getByLabel('Confederación', { exact: true }).selectOption(String(conf.id));
  await dialog.getByLabel('URL del escudo').fill('https://example.test/logo.svg');
  ...
  await dialog.getByRole('button', { name: 'Guardar cambios' }).click();
  await expect(page.getByRole('button', { name: `Editar ${editedName}`, exact: true })).toBeVisible();
  ...
  await expect(page.getByLabel(/Equipo compatible/).locator(`option[value="${visitor.id}"]`)).toHaveCount(0);
  ...
  const afterEnrollment = await (await page.request.get('/api/equipos/')).json();
  expect(afterEnrollment.find(item => item.id === local.id).competiciones.map(item => item.id)).toContain(comp.id);

  await page.getByRole('button', { name: 'Partidos', exact: true }).click();
  await page.getByRole('button', { name: 'Registrar partido', exact: true }).first().click();
  dialog = page.getByRole('dialog');
  await dialog.getByLabel('Competición', { exact: true }).selectOption(String(comp.id));
  await dialog.getByLabel('Equipo local', { exact: true }).selectOption(String(local.id));
  await expect(dialog.getByLabel('Equipo visitante').locator(`option[value="${local.id}"]`)).toHaveCount(0);
  await dialog.getByLabel('Equipo visitante', { exact: true }).selectOption(String(visitor.id));
  await dialog.getByRole('button', { name: 'Guardar cambios' }).click();
  await expect(dialog).toHaveCount(0);
  await expect(page.locator('.v-match-row').filter({ hasText: comp.nombre })).toBeVisible();
  const matches = await (await page.request.get('/api/partidos/')).json();
  expect(matches.some(item => item.competicion_id === comp.id && item.equipo_local_id === local.id && item.equipo_visitante_id === visitor.id)).toBe(true);
  ...
  expect((await context.cookies()).some(item => item.name === 'vertice_token')).toBe(false);
  expect(errors).toEqual([]);
});

test('mobile home, catalogue and modal stay usable with real data', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  const fitsScreen = () => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth);
